...
try:
    cursor.execute("CREATE TABLE myweather (city TEXT, windSpeed TEXT )");
except Exception as E:
    print('Error: ', E) 
else:
    print('Table myweather has been created')

cursor.execute("INSERT INTO myweather VALUES(?,?)", (data['name'], data['wind']['speed']));
dbconnect.commit();
cursor.execute("SELECT * FROM myweather");
for row in cursor:
